-- 🔥 FIRE ANTI LAG: UNIVERSAL EDITION (API IMAGE FIXED) 🔥
-- Forzando carga de ID: 127975661496300
...
local function SafeRead()
    local data = nil
    pcall(function() if isfile and isfile(SaveFile) then data = readfile(SaveFile) end end)
    return data
end
...
-- BOTÓN CON TU ID DE FUEGO (USANDO API DE ROBLOX PARA DECAL)
local OpenBtn = Instance.new("ImageButton", sg)
OpenBtn.Size = UDim2.new(0, 50, 0, 50)
OpenBtn.Position = UDim2.new(0.02, 0, 0.45, 0)
OpenBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
-- Forzamos la carga por el endpoint de asset oficial de Roblox
OpenBtn.Image = "https://www.roblox.com/asset-thumbnail/image?assetId=127975661496300&width=420&height=420&format=png"
...
CreateButton("ULTRA ANTI-LAG", 45, Color3.fromRGB(255, 0, 0), function()
    Settings.UltraAntiLag = true; SafeWrite(); ApplyUltra()
end)

CreateButton("FPS BOOSTER", 90, Color3.fromRGB(255, 0, 0), function()
    Settings.FPSBooster = true; SafeWrite(); ApplyFPS()
end)
...
-- --- CARGA AUTOMÁTICA ---
local savedData = SafeRead()
if savedData then
    local success, decoded = pcall(HttpService.JSONDecode, HttpService, savedData)
    if success then
        Settings = decoded
        if Settings.UltraAntiLag then task.delay(1, ApplyUltra) end
        if Settings.FPSBooster then task.delay(1, ApplyFPS) end
    end
end
